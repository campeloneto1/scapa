...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, TituloComponent, SharedModule, FormularioGraduacoesComponent],
})
export class GraduacoesComponent implements OnInit, OnDestroy {
  //VARIAVEL DAS INFORMCAOES DA PAGINA
  data$!: Observable<Graduacoes>;
  excluir!: Graduacao;
  ...
  @ViewChild(FormularioGraduacoesComponent) child!: FormularioGraduacoesComponent;

  // We use this trigger because fetching the list of persons can be quite long,
  // thus we ensure the data is fetched before rendering
  dtTrigger: Subject<any> = new Subject<Graduacoes>();
  ...
  confirm(id:number){
    this.graduacoesService.destroy(id).subscribe({
      next: (data) => {
