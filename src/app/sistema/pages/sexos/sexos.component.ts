...
import { FormularioSexosCompoennt } from './formulario/formulario-sexos.component';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, TituloComponent, SharedModule, FormularioSexosCompoennt],
})
export class SexosComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(FormularioSexosCompoennt) child!: FormularioSexosCompoennt;
  ...
  confirm(id:number){
    this.sexosService.destroy(id).subscribe({
      next: (data) => {
